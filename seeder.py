    ...
    with Session(engine) as session:
        ...
        try:
            ...
            for i in range(num_dummies):
                users.append(User(username=f"User {i}", email=f"user{i}@example.com", role=random.choice(["user", "admin", "viewer"]), hashed_password=hash_password(f"password{i}"), created_at=datetime.now()))
